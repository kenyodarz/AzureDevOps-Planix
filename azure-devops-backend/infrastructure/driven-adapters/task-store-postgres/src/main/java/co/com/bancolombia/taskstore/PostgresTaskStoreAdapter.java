...
/**
 * Almacén de tareas del BFF sobre PostgreSQL.
 *
 * <p><b>D-20 saldada en la Fase 07.</b> Este adaptador vivía en un módulo llamado
 * {@code task-store-inmemory} que no tenía nada de <i>in-memory</i>: escribe en una tabla real. El
 * nombre llevaba mintiendo desde antes del plan, y un nombre que miente cuesta una lectura de
 * código cada vez que alguien busca dónde se persisten las tareas. El módulo pasa a llamarse
 * {@code task-store-postgres} (DP-05).
 */
@Slf4j
@Component
...
public class PostgresTaskStoreAdapter implements TaskStoreGateway {
    ...
    public Mono<Task> findById(String taskId) {
        ...
        return Mono.fromCallable(() -> {
            ...
            List<Task> results = jdbcTemplate.query(sql, (rs, _) -> {
