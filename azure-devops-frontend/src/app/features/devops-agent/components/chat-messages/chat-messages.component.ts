...
@Component({
  ...
  template: `
    <div
      #chatBox
      class="flex-1 p-8 overflow-y-auto flex flex-col gap-5 w-full max-w-[1000px] mx-auto min-w-0"
    >
      @for (msg of messages; track msg.messageId || $index) {
        <div
          class="flex w-full animate-fadeIn"
          [class.justify-end]="msg.role === 'user'"
          [class.justify-start]="msg.role === 'agent'"
        >
          <div
      ...
      @if (loading) {
        <div class="flex w-full justify-start">
          <div
            class="bg-[rgba(242,201,76,0.08)] border border-[#f2c94c33] rounded-2xl rounded-bl-sm max-w-[90%] p-4 px-5"
          >
  ...
  styles: [
    `
      ...
      ::ng-deep .message-part-container ul,
      ::ng-deep .message-part-container ol {
        margin-left: 20px;
        margin-top: 8px;
        margin-bottom: 8px;
      }
...
export class ChatMessagesComponent implements OnChanges {
  ...
  @ViewChild('chatBox') private readonly chatBoxRef!: ElementRef;
