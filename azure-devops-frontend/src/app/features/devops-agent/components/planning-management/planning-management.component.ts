...
export class PlanningManagementComponent implements OnInit, OnDestroy {
  ...
  protected selectedChunks = signal<PlanningChunk[]>([]);
  ...
  protected deleteInitiative(init: { initiative_id: string; initiative_title: string }): void {
    if (
      confirm(
        `¿Estás seguro de que deseas eliminar la iniciativa "${init.initiative_title}" y todas sus planeaciones vectorizadas? Esta acción no se puede deshacer.`,
      )
    ) {
