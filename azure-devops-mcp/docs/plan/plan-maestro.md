...
> **Fases:** 8 (01 → 08) · **Fase activa:** [`docs/fases/fase-07.md`](../fases/fase-07.md) ·
> **Completadas:** **01, 02, 03, 04, 05, 06**
...
| Métrica                                                        |        Baseline | Fase 02 | Fase 03 | Fase 04 | Fase 05 | Fase 06 |   Objetivo |
|----------------------------------------------------------------|----------------:|--------:|--------:|--------:|--------:|--------:|-----------:|
| Modos de seguridad declarados y probados                       |           **0** | ✅ **2** |   **2** |   **2** | **2** | **2** | **2** *(`permissive` + `enforced`)* |
| Recompilaciones necesarias para pasar a `enforced`             |           **1** | ✅ **0** |   **0** |   **0** |      **0** | **0** |      **0** |
| Tools MCP con autorización declarada **y compilada**           |           **2** | ✅ **8** |   **8** |   **8** |  **8 de 8** | **8 de 8** |  **8 de 8** |
| Anotaciones de seguridad comentadas                            |           **3** | ✅ **0** |   **0** |   **0** |      **0** | **0** |      **0** |
| Rutas abiertas a componentes inexistentes (`/h2-console/**`)   |           **1** | ✅ **0** |   **0** |   **0** |      **0** | **0** |      **0** |
| Secretos con valor por defecto en YAML                         |           **1** | ✅ **0** |   **0** |   **0** |      **0** | **0** |      **0** |
| Arranques posibles sin credencial en modo `enforced`           |         **sí** | ✅ **no** |  **no** |  **no** |     **no** | **no** |     **no** |
| Mecanismos de wiring por bean                                  |           **2** |   **2** |   **2** |   **2** |   **2** | **2** |      **1** |
| Cortacircuitos declarados **sin** configuración correspondiente|           **7** |   **7** |   **7** |   **7** | **3** ▼4 | ✅ **0** ▼3 |      **0** |
| Cortacircuitos del scaffold en el YAML que no usa nadie        |           **2** |   **2** |   **2** |   **2** |   **2** | ✅ **0** |      **0** |
| Líneas de `AzureDevOpsTools`                                   |         **267** | **275** | **277** | **157** ▼121 | **157** | **173** ▲16 *(6 `.onErrorMap` + javadoc; 0 reglas de negocio)* | **≤ 120** |
| Líneas de `RestConsumer` (clase única)                         |         **237** | **237** | ✅ **186** ▼51 | **191** | ✅ **3 adaptadores: 115 / 80 / 78** | **3 adaptadores: 143 / 106 / 112** | **≤ 120 por adaptador** |
| Clases productivas > 200 líneas                                |           **2** |   **2** | ✅ **1** | ✅ **0** |      **0** | **0** |      **0** |
| Reglas de negocio en `entry-points`                            |       **4** | **4** | **4** | ✅ **0** | **0** | **0** | **0** |
| Sentencias WIQL construidas con `String.format`                |           **1** |   **1** |   **1** | ✅ **0 fuera del dominio** | **0** | **0** | **0** |
| Cálculos del año por calendario                                |           **1** |   **1** |   **1** | ✅ **1, medido, con `Clock`** | **1** | **1** | **1, medido y en un solo punto del dominio** |
| Modelos de dominio serializados por Jackson/`WebClient`        |           **3** |   **3** | ✅ **0** |   **0** |      **0** | **0** |      **0** |
| Modelos de dominio deserializados como `@McpToolParam`         |           **2** |   **2** | ✅ **0** |   **0** |      **0** | **0** |      **0** |
| Mappers en la frontera de salida                               |           **0** |   **0** | ✅ **5** |   **5** |  **5**   | **5** |  **≥ 3**   |
| Mappers en la frontera de entrada                              |           **0** |   **0** | ✅ **1** |   **1** |  **1**   | **1** |  **≥ 1**   |
| Value Objects inmutables en `domain/model`                     |           **0** |   **0** |   **0** | ✅ **6** |  **6** | **6** |  **≥ 5** |
| **Excepciones de dominio**                                     |           **0** |   **0** |   **0** |   **0** |  **0** | ✅ **3** *(+1 base)* |  **≥ 3**   |
| **Errores técnicos que llegan crudos al cliente MCP**          |         **todos** | **todos** | **todos** | **todos** |  **todos** | ✅ **0** |  **0** |
| **Traductores de errores en el adaptador (duplicados)**        |             — |       — |       — |       — |        — | ✅ **1 compartido, 0 duplicados** | **1, 0 duplicados** |
| **Timeouts por operación**                                     |           **0** |   **0** |   **0** |   **0** |  **0** | ✅ **4** | **≥ 1 por operación** |
| Puertos por agregado (hoy por operación CRUD)                  |       **7 / 1** | **7 / 1** | **7 / 1** | **7 / 1** | ✅ **3** | **3** | **≤ 3** |
| Puertos huérfanos                                              |           **1** |   **1** |   **1** |   **1** | ✅ **0** | **0** |      **0** |
| Versiones de API hardcodeadas                                  |           **7** |   **7** |   **7** |   **7** | **7** *(en 1 sitio)* | ✅ **2** *(las de B-10, en la ruta)* |      **0** |
| Clases de `domain/model` con `@Setter`                         |          **21** |  **21** |  **21** |  **21** | **21** | **21** | **0** |
| ArchUnit `Rule_2.2` (violaciones reales)                       |           **1** |   **1** | ✅ **0** | ✅ **0** | ✅ **0** | ✅ **0** |      **0** |
| ArchUnit — violaciones **exportadas a Sonar**                  | **0 de 1** *(informe roto, D-25)* | **0 de 1** | **0 de 0** | **0 de 0** | **0 de 0** | **0 de 0** *(D-25 sigue viva)* | **= reales** |
| ArchUnit ejecutado como                                        |     **warning** | **warning** | **warning** | **warning** | **warning** | **warning** | **error**  |
| Módulos sin carpeta de pruebas                                 |           **1** | **1** | **1** | ✅ **0** | **0** | **0** | **0** |
| Tests totales                                                  |          **33** | **67** ▲19 | **77** ▲10 | **148** ▲71 | **148** = | **184** ▲36 |  **≥ 120** ✅ |
| Tests fallando                                                 |           **0** | ✅ **0** | ✅ **0** | ✅ **0** | ✅ **0** | ✅ **0** |      **0** |
| Cobertura `domain/model`                                       | **0 %** | **0 %** | **0 %** | ✅ **94,7 %** | **94,7 %** | **95,8 %** ▲1,1 | **≥ 90 %** ✅ |
| Cobertura `domain/usecase`                                     |      **68,2 %** | **68,2 %** | **68,2 %** | ✅ **97,8 %** | **97,8 %** | **97,8 %** = | **≥ 90 %** ✅ |
| Cobertura `mcp-server` / `rest-consumer` / `app-service`       | **70,5 % / 51,1 % / 17,0 %** | **70,2 % / 90,0 % / 48,4 %** | **73,1 % / 88,1 % / 48,4 %** | **80,7 % / 88,2 % / 58,2 %** | **80,7 % / 88,4 % / 58,2 %** | **83,3 % ▲2,6 / 92,3 % ▲3,9 / 58,2 %** | **≥ 80 / 80 / 60 %** |
| Mutaciones eliminadas (Pitest)                                 |      **5 / 44** | *por remedir* | **19 / 52 (37 %)** | *por remedir* | *por remedir* | **71 / 85 (84 %)** *(`rest-consumer`)* |   **≥ 60 %** |
| Cobertura de `getTeamFieldValues` y `getTeamIterations`        |           **0 %** | **cubiertas** | **cubiertas** | **cubiertas** | **cubiertas** | **cubiertas** | **cubiertas** |
| Complejidad cognitiva máx. por método                          | *por medir* | *por medir* | *por medir* | **≤ 5** | **≤ 5** | **≤ 5** | **≤ 15** ✅ |
...
|-----------|------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| —         | —    | **Ninguna abierta.** Las seis decisiones bloqueantes del plan (DP-01 a DP-06) están **resueltas**. Las Fases 07 y 08 no tienen decisión bloqueante asignada en §5; si alguna aparece, se plantea antes de escribir código conforme a `spring-rules.md` §6. |
...
| 07   | 🔵 **Activa**   | —     | —     | [`fase-07.md`](../fases/fase-07.md) · generada · **sin decisión bloqueante** |
| 08   | ⚪ Pendiente    | —     | —     | — |
